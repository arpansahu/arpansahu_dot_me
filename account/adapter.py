...
def _sanitize_username(username):
    """Sanitize username to not look like an email address.
    
    If the username looks like an email (contains @), extract just the local part.
    This ensures usernames from social auth providers don't expose email addresses.
    """
    if not username:
        return username
    
    # If username looks like an email, take only the part before @
    if '@' in username:
...
class CustomSocialAccountAdapter(DefaultSocialAccountAdapter):
        ...
        if email:
            ...
            try:
                ...
                try:
                    ...
                    text_body = (
                        f"Hi {user.username},\n\n"
                        f"Your {provider_name} account has been connected to your arpansahu.space account.\n"
                    ...
                    _send_mailjet_email(
                        ...
                        f'[arpansahu.space] {provider_name} Account Connected',
                        text_body, html_body,
                    )
    ...
    def save_user(self, request, sociallogin, form=None):
        ...
        if user.email:
            try:
                ...
                text_body = (
                    f"Hi {user.username},\n\n"
                    f"Welcome to arpansahu.space! Your account has been created using {provider_name}.\n\n"
                ...
                _send_mailjet_email(
                    user.email, user.username,
                    f'[arpansahu.space] Welcome, {user.username}!',
                    text_body, html_body,
                )
            except Exception as e:
    ...
    def populate_user(self, request, sociallogin, data):
        ...
        if not user.username:
            ...
            # Sanitize username to not look like an email
            ...
            # Sanitize existing username too (in case it looks like an email)
            user.username = _sanitize_username(user.username)
